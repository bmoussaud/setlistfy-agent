...
resource apiOperationPolicy 'Microsoft.ApiManagement/service/apis/operations/policies@2024-06-01-preview' = [
  for (op, i) in api.operations: if (contains(op, 'policyXml') && !empty(op.policyXml)) {
    name: 'policy'
...
output subscriptionPrimaryKey string = apiSubscriptionRequired
  ? listSecrets('${apimService.id}/subscriptions/api-${toLower(api.name)}', '2024-05-01').primaryKey
  : ''
...
output subscriptionSecondaryKey string = apiSubscriptionRequired
  ? listSecrets('${apimService.id}/subscriptions/api-${toLower(api.name)}', '2024-05-01').secondaryKey
  : ''
